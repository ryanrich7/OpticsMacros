...
for(int ii = 0; ii < 2; ii++){ 
   ...
   if(ii == 0){ qsq[ii] = new TH1F(Form("qsq[%i]",ii),"LHRS Apparent Q^{2} Data",150,0.0,0.015);
                lab[ii] = new TH1F(Form("lab[%i]",ii),"LHRS Apparent #theta_{lab} Data",150,1.0,9.0);
                asym[ii] = new TH1F(Form("asym[%i]",ii),"LHRS Apparent Asymmetry Data",150,0.0,1.0);
                hmom[ii] = new TH1F(Form("hmom[%i]",ii),"LHRS Apparent Asymmetry Data",150,0.938,0.955);
    } else{
    qsq[ii] = new TH1F(Form("qsq[%i]",ii),Form("LHRS Apparent Q^{2} Septum Detuned %.1f",sept),150,0.0,0.015);
    lab[ii] = new TH1F(Form("lab[%i]",ii),Form("LHRS Apparent #theta_{lab} Septum Detuned %.1f",sept),150,1.0,9.0);
    asym[ii] = new TH1F(Form("asym[%i]",ii),Form("LHRS Apparent Asymmetry Septum Detuned %.1f",sept),150,0.0,1.0);
    hmom[ii] = new TH1F(Form("hmom[%i]",ii),Form("LHRS Apparent Momentum Septum Detuned %.1f",sept),150,0.938,0.955);
    }
...
//  T->Add(Form("../prex2Rootfiles/prexLHRS_%i_-1.root",run));
  T->Add(Form("prexLHRS_%i_-1.root",run));

  TChain *T1 = new TChain("T");
//  for(int c = 1; c < 6; c++){ T1->Add(Form("../septmistune/Rootfiles/Ebeam953/Minus1_SandwichLHRS_PREX_%.1f_%i.root",sept,c)); }
//  for(int c = 1; c < 6; c++){ T1->Add(Form("../septmistune/Rootfiles/Ebeam953/Zero1_SandwichLHRS_PREX_%.1f_%i.root",sept,c)); }
  for(int c = 1; c < 6; c++){ T1->Add(Form("../septmistune/Rootfiles/Ebeam953/Plus1_SandwichLHRS_PREX_%.1f_%i.root",sept,c)); }

//  T1->Add(Form("../septmistune/Rootfiles/Minus_SandwichLHRS_PREX_%.1f_1.root",sept));
//T1->Add(Form("../septmistune/Rootfiles/Zero_SandwichLHRS_PREX_%.1f_1.root",sept));
 //T1->Add(Form("../septmistune/Rootfiles/Plus_SandwichLHRS_PREX_%.1f_1.root",sept));
 ...
 la11->SetTextColor(1);
 la12->SetTextColor(2);
 //c1->SaveAs(Form("%s_PolarAngle1_%.1fper.png",TargPos[1],sept));
 ...
 qs1->SetTextColor(1);
 qs2->SetTextColor(2);
// c2->SaveAs(Form("%s_Qsq1_%.1fper.png",TargPos[1],sept));
...
// c3->SaveAs(Form("%s_Asym1_%.1fper.png",TargPos[1],sept));
 ...
 c4->cd(3);
 asym_v->Draw("HIST");
// c4->SaveAs(Form("%s_Vertex1_%.1fper.png",TargPos[2],sept));
